...
class Configuration(object):
    def __init__(self):
        ...
        self.dropout = 0.1
        self.method = 'adam'
        self.FS_hidden_size = 128
        self.SS_hidden_size = 128
...
class NasdaqDataset(Data.Dataset):
    def __init__(self, opt, is_train=True,):
        ...
        self.X = self.preprocess_X_data_1(self.X)
    ...
    def __getitem__(self, item):
        if self.is_train:
            self.return_x = self.X[item: item + self.T - 1, :]
            self.return_y_history = self.y[item: item + self.T - 1]
            self.return_y_target = self.y[item + self.T]
        else:
            self.return_x = self.X[self.train_len + item: self.train_len + item + self.T - 1, :]
    ...
    def preprocess_X_data_1(self, raw_data):
        comparative_X = copy.deepcopy(raw_data)
        for i in range(self.X.shape[0]):
            comparative_X[i, :] = (raw_data[i, :] / raw_data[max(0, i-1)])
        return comparative_X


if __name__ == '__main__':
    opt = Configuration()
    nasdaq = NasdaqDataset(opt)